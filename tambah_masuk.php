...
if ($_SERVER['REQUEST_METHOD']=='POST') {
    ...
    $ins = mysqli_query($conn, "INSERT INTO pemasukan_barang (barang_id,jumlah,tanggal_masuk,supplier,user_id,created_at) VALUES ($barang_id,$jumlah,'$tanggal','$supplier',$user_id,NOW())");
    if ($ins) {
...
?>
<!doctype html><html lang="id"><head><meta charset="utf-8"><title>Input Barang Masuk</title></head>
...
<?php while($r=mysqli_fetch_assoc($barang)) echo '<option value="'.$r['id'].'">'.htmlspecialchars($r['kode_barang'].' - '.$r['nama_barang']).'</option>'; ?>
</select>
...
<label>Supplier</label><input name="supplier" style="width:100%;padding:8px;margin:6px 0">
<button type="submit" style="padding:8px 12px;background:#0d6efd;color:#fff;border-radius:6px;border:none">Simpan</button>
</form>
</div></body></html>
